----doc - https://docs.google.com/document/d/1__Bg43_STnpgkHlUdvF8ELA5rEOBGGD8mUtHSNOMj_M/edit?usp=sharing
----sheet - https://docs.google.com/spreadsheets/d/1JvWTOgp9vNbYNagd2ott7oohw8Lyc1wBuwwvFHKmqbg/edit?usp=sharing
-----Key Result 1 : Keep Square fraud disputes losses under 5.8 bps of card spend
...
------Key Result 2 : Keep fraud dispute rate (count) at or below 6.1 bps of total number of SQ card txns (fraud dispute rates by count)
...
--------Key Result 3 : Keep fraud dispute rate (amount) at or below 19.5 bps of total spend on Square card
...
---------Key Result 4 :  Keep the number of disputed txns. per Seller with disputes under 2.72
with valid_fraud_disputes as (
select *
from bizbank.raw_oltp.transaction_disputes
where reason_code = 'FRAUD_OR_UNAUTHORIZED'
...
--------Key Result 5 : Maintain the number of active SQ card sellers with $ disputes as a % of total spend > 50% under 8 bps
with valid_disputes as (
...
-------- (loss by seller from chipfallback and chip and pin transactions)
 ---------final
  with valid_fraud_disputes as (
select date_trunc('month',transaction_date) as transaction_month,
